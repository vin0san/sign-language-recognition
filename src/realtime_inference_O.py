...
=============================================================
Run this LOCALLY. Requires model.onnx, model.onnx.data, scaler.pkl, label_classes.npy
in model-files/.
...
ONNX_PATH = "model-files/model.onnx"
...
def load_model():
    ...
    print(f"ONNX model loaded — classes: {len(classes)}")
    return sess, scaler, classes
...
def extract_landmarks(lm_list):
    ...
    # Hand-size normalisation: divide by wrist-to-middle-MCP distance
    # This makes features invariant to hand distance from camera
...
@torch.no_grad()
def predict(sess, scaler, vec, classes):
    x     = scaler.transform(vec.reshape(1, -1)).astype(np.float32)
    logits = sess.run(None, {"landmarks": x})[0]
    probs  = np.exp(logits) / np.exp(logits).sum()
    idx    = int(probs.argmax())
    return classes[idx], float(probs.flatten()[idx])
